...
def evaluate_to_type(obj):
    """
    Consumes a dict or list of dicts and iterates over the the iterables
    until it reaches a non-iterable base class at which point it stores the
    type of the value as the value.
    ...
    def parse(self, url=None):
        ...
        for tag in soup.find_all('code'):
            ...
            for tag in table.find_all('tr'):
                ...
                if details.find('a'): # Check for link
                    ...
                    if type(value) == dict or type(value) == list:
                        data[key][attr] = value
                        evaluate_to_type(data[key][attr])
                    else:
...
if __name__ == "__main__":
    g = GithubScraper()
    print json.dumps(g.parse(), indent=4, separators=(',', ':'))
